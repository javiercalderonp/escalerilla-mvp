...
import { formatDelta, type RankingCategory, type RankingEntry } from "@/lib/ranking";
    ...
    <div className="overflow-hidden rounded-2xl border border-slate-200 bg-white shadow-sm">
      ...
      <div className="divide-y divide-slate-100">
        {entries.map((entry) => (
          <Link
            key={entry.id}
            href={`/ranking/${category}#${entry.id}`}
            className="grid grid-cols-[56px_1fr_92px_92px] gap-3 px-4 py-4 transition hover:bg-slate-50 sm:px-6"
          >
            <span className="text-sm font-semibold text-slate-500">{entry.position}</span>
            <div>
              <div className="flex items-center gap-2">
                <span className="text-sm font-medium text-slate-950">{entry.fullName}</span>
