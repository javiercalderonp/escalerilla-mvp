        ...
        <header className="flex items-center gap-3 border-b p-4 pr-12">
          ...
          <div className="min-w-0 flex-1">
            ...
            <div className="mt-1 flex flex-wrap items-center gap-2 text-sm text-muted-foreground">
              ...
              {p.level ? (
                <Badge variant="court">{levelLabel(p.level)}</Badge>
              ) : null}
            </div>
          </div>
        </header>
          ...
          <TabsPanel value="info" className="space-y-3 p-4">
            ...
            {p.backhand ? (
              <InfoRow
                icon={<HandIcon />}
                label="Revés"
                value={p.backhand === "una_mano" ? "Una mano" : "Dos manos"}
              />
            ) : null}
            {p.yearsPlaying != null ? (
              <InfoRow
            ...
            {p.rut ? (
              <InfoRow icon={<IdCardIcon />} label="RUT" value={p.rut} />
            ) : null}
          </TabsPanel>
              ...
              <div className="flex gap-1">
                {data.performance.streak.length > 0 ? (
                  data.performance.streak
                    .map((result, slot) => ({
                      result,
                      slot: data.performance.streak.length - slot,
                    }))
                    .map(({ result, slot }) => (
                      <Badge
                        key={`streak-${slot}-${result}`}
                        variant={result === "W" ? "success" : "warning"}
                        size="sm"
                      >
                        {result}
                      </Badge>
...
function Stat({
  ...
  return (
    <div className="rounded-2xl border border-border bg-card px-3 py-4">
      <p
        className={`text-lg font-semibold tabular-nums ${tone === "grass" ? "text-grass" : "text-foreground"}`}
      >
        {value}
      </p>
