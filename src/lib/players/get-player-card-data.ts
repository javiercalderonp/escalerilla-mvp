...
export async function getPlayerCardData(
  ...
  const recentMatches: PlayerCardData["recentMatches"] = recentMatchesRows.map(
    (match) => {
      const sets = (setsByMatch.get(match.id) ?? []).sort(
  ...
  const matchesWon = recentMatches.filter(
    (match) => match.result === "W" || match.result === "WO_W",
  ).length;
  const matchesLost = recentMatches.filter(
    (match) => match.result === "L" || match.result === "WO_L",
  ).length;
